...
function [simOut, estOut] = singleOmegaSimPred(epiNo, scenNo, nday, I0, delta, gendel, deltype)

% Assumptions and notes
% - includes one-step-ahead predictions and perfect R
...
[~, ~, ~, Rmean, qR] = runEpiSmoother(Rgrid, m, nday, pR, pRup, pstate);
[IRpred, IRci, ape_R, ~] = recursPredictAPE(Rgrid, pR, Lorig, Rmean, Imax, Iday);

% EpiFilter Omega estimation
[~, ~, ~, ~, pOm, pOmup, pOmstate] = runEpiFilter(Rgrid, m, eta, nday, p0, Ipmean, Iday);
[~, ~, ~, Ommean, qOm] = runEpiSmoother(Rgrid, m, nday, pOm, pOmup, pOmstate);
[IOmpred, IOmci, ape_Om, ~] = recursPredictAPE(Rgrid, pOm, Ipmean, Ommean, Imax, Iday);

% EpiFilter with true Lday and hence correctly specified R
[~, ~, ~, ~, pR0, pRup0, pstate0] = runEpiFilter(Rgrid, m, eta, nday, p0, Lday, Iday);
[~, ~, ~, Rmean0, qR0] = runEpiSmoother(Rgrid, m, nday, pR0, pRup0, pstate0);
[IRpred0, IRci0, ape_R0, ~] = recursPredictAPE(Rgrid, pR0, Lday, Rmean0, Imax, Iday);


% Complementary CDFs for R > 1
id1 = find(Rgrid <= 1, 1, 'last'); FqOm(:, i) = 1 - sum(qOm(:, 1:id1), 2)'; 
Fq(:, i) = 1 - sum(qR(:, 1:id1), 2)'; Fq0(:, i) = 1 - sum(qR0(:, 1:id1), 2)'; 
% Means of these CDFs
Fqm = mean(Fq, 2); FqmOm = mean(FqOm, 2); Fqm0 = mean(Fq0, 2);

% Prediction APE scores
tid = delta:nday-1; Itrue = Iday(delta+1:nday); 
apeRsum = sum(ape_R(tid)); apeRsum0 = sum(ape_R0(tid)); apeOmsum = sum(ape_Om(tid));
% Prediction PMSE scores
eIR = IRpred(tid) - Itrue; eIR0 = IRpred0(tid) - Itrue; eIOm = IOmpred(tid) - Itrue;
pmseR = mean(eIR.^2); pmseR0 = mean(eIR0.^2); pmseOm = mean(eIOm.^2);


% Output key estimation variables
estOut.Rmean = Rmean; estOut.Ommean = Ommean; estOut.Rmean0 = Rmean0;
estOut.Fqm = Fqm; estOut.FqmOm = FqmOm; estOut.Fqm0 = Fqm0;
% Output prediction variables
estOut.IRpred = IRpred; estOut.IRpred0 = IRpred0; estOut.IOmpred = IOmpred;
estOut.IRci = IRci; estOut.IRci0 = IRci0; estOut.IOmci = IOmci;
estOut.apeR = apeRsum; estOut.apeR0 = apeRsum0; estOut.apeOm = apeOmsum;
estOut.pmseR = pmseR; estOut.pmseR0 = pmseR0; estOut.pmseOm = pmseOm;